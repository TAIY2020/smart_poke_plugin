...
> 本版本 (v1.3.0) 基于 **MaiBot SDK v2** 开发，使用 `PluginConfigBase` 强类型配置模型，支持配置热重载和 Web UI 配置。**推荐通过 Web UI 修改配置**，所见即所得，不用记字段名。

## ✨ 功能特性

- **被戳反应**: 被戳时按概率随机回戳、发文字、发表情或保持沉默，让麦麦的反应不再机械。
...
主动戳是 v1.3.0 新增的功能——不是按定时器机械触发，而是把"群里有新消息"当成"麦麦醒了一瞬间"，按概率被勾起戳意去戳一下最近说话的熟人。仅群聊触发，私聊不会。
